...
- 兼容：`kernelLearningVersion: 11` 启用 `historyAccumulator`；`kernelLearningVersion: 12` 启用基于已验证 belief samples 的主动信息规划；`kernelLearningVersion: 13` 将信息价值收紧为“后验分支必须改变下一步决策/预期变化”；旧版本 Replay 移除不属于其版本的字段，不改变旧上下文和旧规划语义。
...
- 实现：`kernelLearningVersion: 13` 的规划读取候选动作当前关系下最多 8 个 `beliefModels` 后验样本，只在第一步做有界结果分支；每个分支仍通过同一关系签名重新生成安全候选，并且只有分支后的下一步动作 token 或预期变化真正不同，才把当前不确定度与分支后不确定度的下降计入规划效用。无 belief samples 时保持均值推演；Replay 对 12 之前的 STEP 强制 legacy 均值语义，对 v12 保留旧的“仅看不确定度”规则。Kernel 不接收隐藏状态、领域字段或信息增益标签。
