...
- 结论：上下文条件化的协议、学习、重启恢复和 Replay 已经成立，但在当前 WorldPort、取证策略和 96 步窗口下，两个策略不可区分，不能把“更具体的记忆”当成智能本身。下一轮应先验证上下文键的可复用性、样本覆盖和探测策略，再决定是否调整模型结构。R26 报告绑定源码指纹 `a4142d3f582c38988f174d1847219b4572c83e0ed62e0cb9c380a3824f8b5390`。
...
- 结果：8/8 Run 的 Replay 为 `CONSISTENT`，每次产生 `281` 个模型，其中可复用模型为 `0`；h0/h1/h2 分别为 `93/95/93` 个模型，全部只有 1 个样本。
- 结论：当前上下文键把完整实际变化向量当作身份，导致历史中的微小变化把键持续打散。问题已经从“是否增加记忆”收敛为“如何在不引入领域字段的情况下定义稳定的上下文等价关系”。下一步应测试有界的方向/区间抽象，再测其误合并和收益代价。R27 报告绑定源码指纹 `cc8952e53043f2a4070a637c000ba3ae5347f0d0efdade8d98ad6b6c2ec6ba4b`。
