...
- 边界：负结果说明即使模型确实读取了全局 proposal 模型，跨时段/SOC 的平均值仍不足以表达连续控制；不证明 proposal 条件化记忆无用，也不证明任何真实电费策略。下一轮的最小变化应增加 proposal × context 的有界模型，并用同 seed 对照检验是否消除这类混淆；不能直接把全局均值改成新的领域权重。R25 报告绑定源码指纹 `67e12d37c193589b0ea25e46fcdf5ffe2ad19fd819c6c1fb27649d2ad832dbb9`。
...
- 结论：方向抽象后，上下文条件化已经能改变策略，但质量不稳定：8 对中改善 `5` 对、变差 `3` 对，平均仍多成本 `18.75` 元。协议、学习、重启恢复和 Replay 已经成立，但不能把“更具体的记忆”当成智能本身；下一轮应验证最小样本与不确定性门槛。R26 报告绑定源码指纹 `fd1304f4acd447c58303da12737e0f9738d7d44e1ef1f5106deb43a4456c20ec`。
...
- 结果：8/8 Run 的 Replay 为 `CONSISTENT`，平均产生 `200.75` 个模型，其中平均 `38.25` 个达到至少 2 个样本；h0/h1/h2 平均分别为 `37/73/91` 个模型，可复用数分别为 `21/16/2`。
- 结论：不引入领域字段的方向抽象显著提高了上下文复用，但仍有大量一次性键，尤其是 h2。下一步应测试最小样本门槛和不同上下文窗口的收益、误合并代价。R27 报告绑定源码指纹 `fc5af1b5fa09964f81dccbb3012b4a0112aaa495911ad3bf6be70b674927e7af`。
