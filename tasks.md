...
- 兼容：`kernelLearningVersion: 11` 启用 `historyAccumulator`；`kernelLearningVersion: 12` 启用基于已验证 belief samples 的主动信息规划；旧版本 Replay 移除不属于其版本的字段，不改变旧上下文和旧规划语义。
...
- 实现：`kernelLearningVersion: 12` 的规划读取候选动作当前关系下最多 8 个 `beliefModels` 后验样本，只在第一步做有界结果分支；每个分支仍通过同一关系签名重新生成安全候选，并把当前不确定度与分支后第一项决策不确定度的下降计入规划效用。无 belief samples 时保持原有均值推演；Replay 对 12 之前的 STEP 强制 legacy 均值语义。Kernel 不接收隐藏状态、领域字段或信息增益标签。
- 边界：这是从已验证数值后验和关系分区得到的主动实验启发式，不是完整 belief-state、熵、因果或 POMDP 求解；分支样本有限、未来记忆不在模拟中写回，不能证明探测一定有效，也不能越过 WorldPort 的真实安全/授权边界。
- 验证：二维同观测反例中，单步贪心选择利用动作，有界规划选择探测动作；既有 detour、最大能力面、CLI、多 WorldPort、重启恢复和 Replay 继续验证。
