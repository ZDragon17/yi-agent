...
- 实现：ess-arbitrage adapter 新增 `--chain-credit` 模式（独立分支，不与 settlement-delay 叠加）：charge 步归因窗口未完成（收益延迟到峰时放电）；首个 discharge 登记链 `[charge 0.5, discharge 0.5]` 并保 pending；链反馈在两成员都 pending 之后的下一步发出（协议要求：当前步的 pending 在结算之后才建立——首版在放电步自引用即被 Kernel fail-closed 拦截，修正为下一步发出）。`scripts/curriculum/r11-chain-gain.mjs` 以链开/关 × horizon 1/4/8 各 96 步对照。
- 结果（混合，如实记录）：(1) 相对收益显著——同 seed 下链 h8 缺口 +7.15% → +0.07%（待机基线平价，7.08pp），h4 +7.15% → +0.92%；ACTION_CHAIN 成对结算 28-34 次/96 步，全部 Replay 一致。(2) 规划交互——h1 下链略有害（+3.52% → +4.51%，窗口级 delta 误导单步策略），h4/h8 下大幅有益：信用链恰好在规划能利用它的地方产生收益。(3) 种子敏感性——无链配置跨 seed 缺口 +7.15% 到 -3.4%（高方差），链配置聚在 +0.07% 到 -2.3%；个别 seed 下链已低于待机基线（净套利出现）。(4) 预注册正阈值（gap ≤ -8%）未达：跨 seed 稳定的净套利仍未实现。
- 验证：E2E 两项——3 seed × 链/无链 × h8 的均值断言（链均值严格优于无链 + 每次链运行 ≥20 成对链结算 + 全部 Replay 一致）与链时序边界（锚点必须属于更早步骤）；既有 curriculum level4/5 回归 14/14 通过；164 提交线上基线全量 584/584 通过。
- 边界：份额 0.5/0.5 为世界声明的启发式（F-133/F-139 已证明错误份额与共谋不可由协议识别）；净套利的跨 seed 稳定性、链长 >2 的扩展（一充多放/多充一放）、以及与 v30-v32 因果证据的组合是下一轮磨练入口。R1 缺口曲线的台阶结构现在有了机制解释的下半段：信用链补上了「谷充动作的延迟收益归因」，但「把归因转成稳定低于基线的调度」仍差策略层一步。

## F-257 链长效应——多充链的负结果与 pair 链稳定净套利（R12）

- 缺口：F-256 证明 pair 链（1充1放）在 h8 下收敛到基线平价、个别 seed 净套利，但跨 seed 稳定净套利未达；真实日形态（谷时多充+峰时一放）需要链长 >2——协议支持但从未检验。
- 实现：ess-arbitrage 链模式扩展 `--chain-max-charges N`（1-3）：充电步累积 nonce（有界列表），放电消费全部并登记链 `[charge1..chargeN, discharge]`，份额二进制精确构造（前 N-1 个 0.25、末成员余数——kernel 闭合容差 1e-9，均分 1/(N+1) 舍入会 fail-closed）。`scripts/curriculum/r12-chain-length.mjs`：3 seed × {base, pair, multi3} × h8 × 96 步。
- 结果（双判据失败 + 意外强化，如实记录）：(1) **pair 链跨 seed 稳定净套利**——均值 -2.66%（13197/13327/13312 vs 基线 13642，方差 ±1%）：R1 缺口曲线的核心开放问题（信用分配能否转为净策略收益）首次得到稳定肯定回答，尽管只达 ~13% 潜力的 1/5。(2) **多充链负结果**——multi3 均值 +0.97% 显著差于 pair 的 -2.66%，主判据（multi<pair）与冲关判据（multi<0）均失败。机制线索：份额稀释（每充 0.25 vs pair 的 0.5）与更长窗口携带更多负荷曲线噪声；multi3 链结算更多（37-39 vs 24-26 对）且孤儿更少（3-5 vs 9-16）却更差——排除孤儿损失解释，指向信用信号质量。链长存在最优值，更长≠更好（假设，未独立验证）。
- 验证：E2E 锁定 pair 稳定净套利（3 seed 均值低于基线 ≥1% + 链结算 ≥20）与 multi3 协议正确性（4 成员链出现并结算 + Replay 一致）；性能排序不锁死为断言（未来学习改进可能改变，JSON 存档）；九配置全部 Replay CONSISTENT。
- 边界：份额为世界声明的启发式（等分），物理最优份额未探索；-2.66% 距 13% 全潜力仍差一个数量级——策略层（如何把信用模型转成更激进的调度）仍是缺口；多充链的窗口噪声假设可用受控窗口长度实验独立检验。
...
- 结果：pair 均值 `13279`（gap `-2.66%`），multi2 `13585`（`-0.42%`），multi3 `13774`（`+0.97%`），baseline `13642`。multi2 三个 seed 都出现并结算了 3 成员链（21-32 条），全部 Replay CONSISTENT；结果落在 pair 与 multi3 之间，支持“链长非单调、可能存在中间最优”的假设，但不证明全局最优。
- 验证：R12 E2E `1/1` 通过，覆盖 9 条 pair/multi2/multi3 运行；脚本覆盖 12 条 base/pair/multi2/multi3 运行并逐条 Replay CONSISTENT。没有修改 Kernel，也没有把当前排序固化成行为契约。
- 边界：当前样本只有 3 个 seed，multi2 的收益仍未达到稳定净套利阈值；策略层仍未把链因果信用转化为独立的调度信号。下一步应优先做独立随机化/因果检验或策略层信号实验，而不是继续盲目增加链长。

## F-259 价值投影与链信用的交互（R14：累计效用负结果）
