...
test('current loop continuation recovery remains readable when array run materialization is unavailable', async () => withLab(async ({ lab }) => {
  const { LabStore } = await loadRuntime();
  const store = await LabStore.init(initOptions(lab));
  const continuation = {
    schemaVersion: SCHEMA_VERSION,
    loopId: '00000000-0000-4000-8000-000000000008',
    scenario: 'steady',
    runIndex: 0,
    stepsPerRun: 1,
  ...
  const recovered = await streamOnlyStore.readCurrentLoopContinuation();
  assert.equal(recovered.loopId, continuation.loopId);
  assert.equal(recovered.nextRunIndex, 1);
  assert.equal(recovered.status, 'ACTIVE');
}));
