...
const WORLD_ID = 'latent-choice';
const CAPABILITY_IDS = ['latent-choice.a', 'latent-choice.b'];
...
const hiddenMode = modeIndex === -1 ? null : process.argv[modeIndex + 1] ?? null;
if (stateFile === null || !['A', 'B'].includes(hiddenMode)) throw new Error('state-file and mode are required');
...
function dispatch(op, payload) {
  if (op === 'hello') {
    const descriptor = {
      adapterId: `latent-choice-adapter-${hiddenMode.toLowerCase()}-v1`,
      worldId: WORLD_ID,
      worldVersion: 'latent-choice-1',
...
function transition(prior, request) {
  ...
  if (prior.revision !== stored.revision || prior.value !== stored.value) throw new Error('transition state does not match durable latent state');
  ...
  const delta = hiddenMode === 'A' ? (actionA ? 1 : -1) : (actionA ? -1 : 1);
  const next = {
...
function stateFromStore(source) {
  return {
    schemaVersion: VERSION,
    stateVersion: `latent-choice:${source.revision}`,
...
function observation(state) {
  return {
    ...
    intervalId: `latent-choice:${state.revision}`,
    evidence: [],
  };
}
