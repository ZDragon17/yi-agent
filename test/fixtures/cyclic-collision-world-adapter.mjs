...
const WORLD_ID = 'cyclic-collision';
...
// 周期-7 赢家调度：A,B,A,C,B,A,D。窗口-1 键在 alpha 处三分歧，窗口-2 键的
// (B,A) 对在 C 与 D 两个相位间二分歧；只有至少覆盖一个完整周期的上下文窗口
// 才能表达该相位条件策略。
...
function dispatch(op, payload) {
  if (op === 'hello') {
    const descriptor = {
      ...
      worldId: WORLD_ID,
      worldVersion: 'cyclic-collision-1',
...
function transition(prior, request) {
  ...
  const next = {
    ...
    phase: (prior.phase + 1) % WINNER_SCHEDULE.length,
...
// 隐藏相位只存在于世界内部：每步推进一格；赢家 +1，相位次者 0，其余 -1。
// 观测向量不携带任何相位信息。
function deltaForPhase(phase, actionIndex) {
  const winner = WINNER_SCHEDULE[phase % WINNER_SCHEDULE.length];
  if (actionIndex === winner) return 1;
  if (actionIndex === (winner + 1) % CAPABILITY_IDS.length) return 0;
  return -1;
}
...
function stateFromStore(source) {
  return {
    schemaVersion: VERSION,
    stateVersion: `cyclic-collision:${source.revision}`,
...
function observation(state) {
  return {
    ...
    intervalId: `cyclic-collision:${state.revision}`,
    evidence: [],
  };
}
