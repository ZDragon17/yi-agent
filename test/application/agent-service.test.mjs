...
test('bounded planning is persisted at the step boundary and survives replay', async () => {
  await withLab(async (lab) => {
    ...
    assert.deepEqual(step.payload.boundary.planning, { schemaVersion: 1, horizon: 2, contextMode: 'context-v1' });
    assert.equal(step.payload.boundary.kernelLearningVersion, 16);
    assert.equal((await replayLab({ labPath: lab, runId: 'run-1' })).verdict, 'CONSISTENT');
  });
});
...
test('continuous planning configuration survives interruption and resume', async () => {
  await withLab(async (lab) => {
    ...
    assert.deepEqual(run.events.find((event) => event.kind === 'STEP').payload.boundary.planning, { schemaVersion: 1, horizon: 2, contextMode: 'context-v1' });
