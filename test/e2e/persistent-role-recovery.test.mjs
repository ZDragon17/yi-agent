...
test('persistent observer response loss resumes without repeating the authority effect', async () => {
  const root = await mkdtemp(path.join(tmpdir(), 'yi-agent-persistent-observer-recovery-'));
  try {
    const lab = path.join(root, 'lab');
    const adapter = path.join(root, 'adapter.json');
    const effectFile = path.join(root, 'primary-effect.json');
    ...
    await writeFile(adapter, JSON.stringify({
      ...
      executionAuthority: {
        executable: process.execPath,
        args: [ADAPTER, '--effect-file', effectFile, '--execution-authority', '--authority-effect-file', authorityEffectFile],
        adapterId: 'idempotent-execution-authority-v1',
        worldId: 'idempotent-transition',
        timeoutMs: 5000,
        transport: 'persistent-jsonl',
      ...
      executionObserver: {
        executable: process.execPath,
        args: [ADAPTER, '--effect-file', effectFile, '--execution-observer', '--observer-drop-marker', observerDropMarker],
        adapterId: 'idempotent-execution-observer-v1',
        worldId: 'idempotent-transition',
        timeoutMs: 5000,
        transport: 'persistent-jsonl',
    ...
    const init = await invoke([
      'init', '--lab', lab, '--world', 'idempotent-transition', '--seed', 'persistent-observer-recovery-seed',
      '--lab-id', 'persistent-observer-recovery-lab', '--adapter', adapter, '--json',
    ]);
    assert.equal(init.code, 0, JSON.stringify(init));
...
test('persistent authority and observer response losses recover in sequence with one effect', async () => {
  const root = await mkdtemp(path.join(tmpdir(), 'yi-agent-persistent-role-chain-recovery-'));
  try {
    const lab = path.join(root, 'lab');
    const adapter = path.join(root, 'adapter.json');
    const effectFile = path.join(root, 'primary-effect.json');
    ...
    const init = await invoke([
      'init', '--lab', lab, '--world', 'idempotent-transition', '--seed', 'persistent-role-chain-recovery-seed',
      '--lab-id', 'persistent-role-chain-recovery-lab', '--adapter', adapter, '--json',
    ]);
    assert.equal(init.code, 0, JSON.stringify(init));

    const authorityLost = await invoke([
      'run', '--lab', lab, '--run-id', 'run-1', '--steps', '1', '--scenario', 'idempotent', '--adapter', adapter, '--json',
    ]);
    assert.notEqual(authorityLost.code, 0, JSON.stringify(authorityLost));
    assert.equal(authorityLost.json.error.code, 'WORLD_ADAPTER_PROTOCOL');
    assert.equal(JSON.parse(await readFile(effectFile, 'utf8')).effectCount, 1);
    assert.equal(JSON.parse(await readFile(authorityEffectFile, 'utf8')).effectCount, 1);
    ...
    assert.equal(observerLost.json.error.code, 'WORLD_ADAPTER_PROTOCOL');
    assert.equal(JSON.parse(await readFile(effectFile, 'utf8')).effectCount, 1);
    assert.equal(JSON.parse(await readFile(authorityEffectFile, 'utf8')).effectCount, 1);

    const resumed = await invoke([
      'run', '--lab', lab, '--run-id', 'run-3', '--steps', '1', '--scenario', 'idempotent', '--adapter', adapter, '--json',
    ]);
    assert.equal(resumed.code, 0, JSON.stringify(resumed));
    assert.equal(JSON.parse(await readFile(effectFile, 'utf8')).effectCount, 1);
    assert.equal(JSON.parse(await readFile(authorityEffectFile, 'utf8')).effectCount, 1);
