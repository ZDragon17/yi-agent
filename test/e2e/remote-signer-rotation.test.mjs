...
test('mTLS certificate revocation rejects the old client before nonce recovery', async () => {
  const root = await mkdtemp(path.join(tmpdir(), 'yi-agent-signer-revocation-'));
  let signer;
  try {
    const lab = path.join(root, 'lab');
    const effectFile = path.join(root, 'primary-effect.json');
    ...
    const clientKey = path.join(root, 'client.key.pem');
    const clientCert = path.join(root, 'client.crt.pem');
    const newClientKey = path.join(root, 'client-new.key.pem');
    const newClientCert = path.join(root, 'client-new.crt.pem');
    const caKey = path.join(root, 'test-ca.key.pem');
    const caCert = path.join(root, 'test-ca.crt.pem');
    const crlPath = path.join(root, 'test-ca.crl.pem');
    const markerName = `${canonicalDigest('execution:step:1').slice('sha256:'.length)}.marker`;
    await mkdir(path.join(sandboxRoot, 'pending'), { recursive: true });
    await mkdir(path.join(sandboxRoot, 'applied'), { recursive: true });
    await writeFile(path.join(sandboxRoot, '.yi-agent-sandbox'), 'yi-agent-sandbox-v1\n', 'utf8');
    ...
    const { privateKey, publicKey } = generateKeyPairSync('ed25519');
    await writeFile(privateKeyPath, privateKey.export({ format: 'der', type: 'pkcs8' }));
    await writeFile(authTokenPath, 'remote-signer-revocation-token-2026');
    const authority = await createCertificateAuthority(caKey, caCert, 'yi-test-ca-revocation');
    await makeCertificateSignedByAuthority(authority, serverKey, serverCert, 'localhost', 1);
    await makeCertificateSignedByAuthority(authority, clientKey, clientCert, 'yi-authority', 2);
    await makeCertificateSignedByAuthority(authority, newClientKey, newClientCert, 'yi-authority', 3);
    await makeCertificateRevocationList(authority, crlPath, [2]);
    const executionPublicKey = publicKey.export({ format: 'der', type: 'spki' }).toString('base64');
    signer = startSigner({
      privateKeyPath,
      authTokenPath,
    ...
    const signerPort = Number(await readFile(readyPath, 'utf8'));
    const descriptor = {
      adapterId: 'effect-broker-authority-revocation-v1',
      worldId: 'idempotent-transition',
      worldVersion: 'idempotent-transition-1',
      capabilityIds: ['idempotent-transition.advance'],
      scenarioIds: ['idempotent', 'alternate'],
    ...
    const init = await invoke(['init', '--lab', lab, '--world', descriptor.worldId, '--seed', 'signer-revocation-seed', '--lab-id', 'signer-revocation-lab', '--adapter', adapter, '--json']);
    assert.equal(init.code, 0, JSON.stringify(init));
    const revoked = await invoke(['run', '--lab', lab, '--run-id', 'run-1', '--steps', '1', '--scenario', 'idempotent', '--adapter', adapter, '--json']);
    assert.notEqual(revoked.code, 0, JSON.stringify(revoked));
    assert.equal(JSON.parse(await readFile(effectFile, 'utf8')).effectCount, 1);
    assert.equal(await readFile(path.join(sandboxRoot, 'applied', markerName), 'utf8'), 'execution:step:1');
    signer.kill();
    await waitForExit(signer);

    await writeFile(clientKey, await readFile(newClientKey));
    await writeFile(clientCert, await readFile(newClientCert));
    await rm(readyPath, { force: true });
    signer = startSigner({
      privateKeyPath,
      authTokenPath,
...
function startSigner({ privateKeyPath, authTokenPath, serverKeyPath, serverCertPath, clientCaPath, serverCrlPath, readyPath, port, dropResponseMarker }) {
  const args = [
    SIGNER_SERVER,
  ...
  if (serverCrlPath !== undefined) args.push('--tls-crl-file', serverCrlPath);
