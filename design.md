...
- 候选历史不是连续性 Memory 的一部分；Runtime 从已提交终态 Run 的 STEP 中投影最近 32 条 `candidateHistory`，每条保留 `runId,worldId,scenario,sequence,recordedAt,candidateOutcome`，以及可选的有界 `proposal/proposalDigest/proposalTruncated`。单个 proposal 预览最多 8 KiB，ModelAdvisor 侧历史总预算最多 32 KiB；超限候选保留摘要并标记截断。它可在 `inspect` 和下一次 ModelAdvisor 提示中读取，跨进程/跨 Run 恢复；活动 Run 的未终态前缀不作为历史来源，避免把未提交事实泄露给下一决策。
