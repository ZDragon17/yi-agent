...
| `WorldRegistry` | world id、稳定 worldVersion、manifest、scenario 和 state version | WorldPort、ValueSpec、声明式 externalInputs | Application 的适配边界；新实验把 worldVersion 固化进 manifest，继续运行/恢复/Replay 必须校验同一版本；未知世界只能通过显式 registry 注入，外部输入必须成为可摘要、可重放的证据 |
...
- `manifest.json`：初始化后不可变，含 schemaVersion、labId、worldId、seed、createdAt、canonicalRoot、稳定的 `worldVersion`、声明式 `scenarioIds`、稳定 tokenMap 及 digest；新实验的继续运行、重启恢复和 Replay 必须校验注册表中的 WorldPort 版本与 manifest 一致。缺少 `worldVersion` 的历史 manifest 仅按 legacy 兼容路径读取。外部 adapter 另含 `{schemaVersion:1,protocol:"yi-world-cli",version:1,adapterId,worldVersion,valueSpec,evidencePublicKey,descriptorDigest,launchDigest,supportsIdempotentTransitions?}`，并与顶层 worldVersion 对齐，将协议/描述/启动材料、幂等恢复声明和外部证据公钥绑定到实验空间。tokenMap 为 `{schemaVersion:1,entries:[{token,capabilityId}],digest}`，应用层使用由 `SHA256(seed,labId,"capability-map",capabilityId)` 派生的独立 token 域，不消耗 world/policy RNG；即使 seed 相同，不同 labId 也产生不同映射。Runtime 只依据 manifest 的场景契约校验场景标识，不内置新的 WorldPort 领域名单。
