...
| `runContinuous(input)` | lab、每 Run 步数、Run 数上限或 persisted continuation | 多个已提交 Run 的汇总 | 每个 Run 的 loopId/runIndex/scenario/budget 固化在 immutable start；同一 lab 的未完成 continuation 在 writer lock 内原子地排他，且只接受与持久化 `nextRunIndex` 相等的下一逻辑 Run；resume 从完整账本重建 nextRunIndex；当前调用遇到终止原因即停止串联，目标达成完成 continuation，崩溃和可幂等外部不确定保留恢复入口，不把失败伪装成持续成功 |
