...
- 候选历史不是连续性 Memory 的一部分；Runtime 从已提交终态 Run 的 STEP 中投影最近 32 条 `candidateHistory`，每条保留 `runId,worldId,scenario,sequence,recordedAt,candidateOutcome`。它可在 `inspect` 和下一次 ModelAdvisor 提示中读取，跨进程/跨 Run 恢复；活动 Run 的未终态前缀不作为历史来源，避免把未提交事实泄露给下一决策。
