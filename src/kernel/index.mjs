...
const INDEPENDENT_CAUSAL_CREDIT_EVIDENCE_LEARNING_VERSION = KERNEL_LEARNING_VERSIONS.independentCausalCreditEvidence;
...
const CREDIT_CHAIN_KEYS = ['schemaVersion', 'basis', 'members', 'attestation', 'independentAttestation'];
...
export function learn(input) {
  ...
  const settlement = settlePendingCredits(
    ...
    learningVersion >= INDEPENDENT_CAUSAL_CREDIT_EVIDENCE_LEARNING_VERSION,
  );
...
function settlePendingCredits(
  ...
  for (const item of orderedFeedback) {
    ...
    if (item.creditChain.basis === INDEPENDENT_CAUSAL_CREDIT_BASIS && !allowIndependentCredit) {
      contractViolation('kernel independent causal credit evidence requires its learning version', {
        field: `${field}.${item.executionNonce}.creditChain.basis`,
      });
    }
    ...
    for (const member of members) {
      ...
      previousOrder = order;
      if (item.creditChain.basis === undefined) {
    ...
    if (item.creditChain.basis === undefined && Math.abs(shareTotal - 1) > CREDIT_CHAIN_SHARE_TOLERANCE) {
  ...
  for (const item of orderedFeedback) {
    ...
    if (creditChain !== undefined) {
      ...
      hasFeedbackSettlement = true;
      rememberSettledFeedback(memory, item);
      if (chainSettlement.learnable) cleanDeltas.push(chainSettlement.actualDelta);
...
function settleCreditChain(memory, feedback, members, pendingByNonce, dimensions, field, refreshModelAge) {
  ...
  if (feedback.creditChain.basis === CAUSAL_CREDIT_BASIS || feedback.creditChain.basis === ATTESTED_CAUSAL_CREDIT_BASIS || feedback.creditChain.basis === INDEPENDENT_CAUSAL_CREDIT_BASIS) {
    const observed = Array.from({ length: dimensions }, () => 0);
    for (const member of members) {
      for (let index = 0; index < dimensions; index += 1) {
        observed[index] = assertComputedFiniteNumber(
          observed[index] + member.delta[index],
          `${field}.${feedback.executionNonce}.creditChain.observedDelta[${index}]`,
        );
      }
    }
    const mismatch = actualDelta.some((value, index) =>
      Math.abs(value - observed[index]) > CREDIT_CHAIN_SHARE_TOLERANCE);
    if (mismatch) {
      const error = subtractVectors(
        actualDelta,
        anchor.expectedDelta,
        `${field}.${feedback.executionNonce}.creditChain.error`,
      );
      return {
        actualDelta,
        learnable: false,
        entries: members.map((member) => ({
          schemaVersion: SCHEMA_VERSION,
          executionNonce: member.executionNonce,
          token: pendingByNonce.get(member.executionNonce).token,
          attribution: 'AMBIGUOUS',
          confidence: 0,
          learnable: false,
          error: cloneVector(error),
          reason: 'CAUSAL_EVIDENCE_MISMATCH',
        })),
      };
    }
    const entries = [];
...
function normalizeFeedback(value, field, dimensions) {
  ...
  return items.map((item, index) => {
    ...
    const creditChain = source.creditChain === undefined
      ? undefined
      : normalizeCreditChain(source.creditChain, `${field}[${index}].creditChain`, dimensions);
    return {
...
function normalizeCreditChain(value, field, dimensions) {
  ...
  if (basis !== undefined && basis !== CAUSAL_CREDIT_BASIS && basis !== ATTESTED_CAUSAL_CREDIT_BASIS && basis !== INDEPENDENT_CAUSAL_CREDIT_BASIS) {
    contractViolation('kernel credit chain basis is unsupported', {
      field: `${field}.basis`,
      actual: basis,
    });
  }
  const attestation = basis === ATTESTED_CAUSAL_CREDIT_BASIS || basis === INDEPENDENT_CAUSAL_CREDIT_BASIS
  ...
  const independentAttestation = basis === INDEPENDENT_CAUSAL_CREDIT_BASIS
    ? normalizeCreditAttestation(source.independentAttestation, `${field}.independentAttestation`)
    : source.independentAttestation === undefined
      ? undefined
      : contractViolation('kernel non-independent credit chain cannot contain independent attestation', {
          field: `${field}.independentAttestation`,
        });
  ...
  const normalizedMembers = members.map((item, index) => {
    ...
    const member = assertPlainRecord(item, itemField, CREDIT_CHAIN_MEMBER_KEYS, basis === undefined
      ? ['executionNonce', 'share']
      : ['executionNonce', 'delta']);
    const executionNonce = assertBoundedString(
    ...
    if (basis !== undefined) {
      return {
        executionNonce,
        delta: assertFiniteVector(member.delta, `${itemField}.delta`, dimensions),
      };
    }
  ...
  if (basis === undefined && Math.abs(shareTotal - 1) > CREDIT_CHAIN_SHARE_TOLERANCE) {
    contractViolation('kernel credit chain shares must sum to one', {
      field: `${field}.members`,
      actual: shareTotal,
    });
  ...
  return {
    ...
    ...(independentAttestation === undefined ? {} : { independentAttestation }),
...
function creditChainEqual(left, right) {
  ...
  return left.schemaVersion === right.schemaVersion &&
    ...
    (left.independentAttestation === undefined || right.independentAttestation === undefined
      ? left.independentAttestation === right.independentAttestation
      : left.independentAttestation.schemaVersion === right.independentAttestation.schemaVersion &&
        left.independentAttestation.digest === right.independentAttestation.digest &&
        left.independentAttestation.attestation === right.independentAttestation.attestation) &&
    left.members.length === right.members.length &&
    left.members.every((member, index) =>
      member.executionNonce === right.members[index].executionNonce &&
      (left.basis === undefined
        ? Object.is(member.share, right.members[index].share)
        : member.delta.length === right.members[index].delta.length &&
          member.delta.every((value, deltaIndex) => Object.is(value, right.members[index].delta[deltaIndex]))));
}
  ...
  return {
    ...
    ...(value.creditChain === undefined ? {} : {
      creditChain: {
        ...
        ...(value.creditChain.independentAttestation === undefined ? {} : {
          independentAttestation: {
            schemaVersion: value.creditChain.independentAttestation.schemaVersion,
            digest: value.creditChain.independentAttestation.digest,
            attestation: value.creditChain.independentAttestation.attestation,
          },
        }),
        members: value.creditChain.members.map((member) => ({
          ...
          ...(value.creditChain.basis === undefined
            ? { share: member.share }
            : { delta: cloneVector(member.delta) }),
        })),
      },
    }),
  };
