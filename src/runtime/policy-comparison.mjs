...
export function comparePairedPolicies(input) {
  if (!isRecord(input) || !isNonEmptyText(input.worldVersion) || !isDigest(input.tokenMapDigest) ||
      !isNonEmptyText(input.scenario) || !isDigest(input.initialStateDigest) ||
      !isPolicyEvidence(input.left) || !isPolicyEvidence(input.right) ||
      input.left.policyDigest === input.right.policyDigest) {
    return null;
  }
  ...
  return {
    ...
    metric: metric.name,
    leftPolicyDigest: input.left.policyDigest,
    rightPolicyDigest: input.right.policyDigest,
    leftTraceDigest: input.left.traceDigest,
    rightTraceDigest: input.right.traceDigest,
...
function isPolicyEvidence(value) {
  return isRecord(value) && isDigest(value.policyDigest) && isDigest(value.traceDigest) &&
    isDigest(value.terminalDigest) && isRecord(value.quality);
}
