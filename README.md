...
- F-184 收紧 F-183 的配置边界：主 WorldPort 与 `reconciliationObserver` 不能复用完全相同的可执行文件、参数和 transport；宿主在第一次 `hello` 前直接拒绝这种“只换身份名”的配置。对账与账本/Replay 组合回归 `91/91`，该约束仍只排除配置层面的假分离，不等于 OS 权限、跨机器身份或现实效果证明。
