...
- 显式动作链信用（`kernelLearningVersion: 29`）：WorldPort 可在单条 clean feedback 上声明 `creditChain:{schemaVersion:1,members:[{executionNonce,share}]}`，Kernel 要求链以反馈 nonce 为锚、成员按 pending 顺序排列、全部仍在 pending、份额闭合为 1 且不与同批反馈重叠；它把锚点动作前到反馈快照的净变化按份额分配给整条动作链，输出 `ACTION_CHAIN` 并随 Replay 重建。WorldPort 不能把未列入 members 的动作效果混入快照；若当前 transition 同时执行新动作，反馈通常应截在该动作执行前。Kernel 校验结构和版本，不验证现实因果或快照时序；旧 feedback 仍按 v28 及更早语义运行；
...
只读 repo WorldPort 证明了“理解和验证”能进入账本，但还没有证明真实文件改动能安全跨过 `transition→verify→ledger→replay`。因此 adapter 另有显式的 writable 模式：在配置中追加补丁策略文件和 nonce 日志路径后，能力集合才增加 `repo.apply-patch`。策略文件支持两种范围：`targetPath + expectedBeforeDigest` 固定一个目标，或 `allowedPaths` 声明一组候选目标；后一种模式必须同时启用 `--discover`，每个目标都要出现在当前文件列表中，摘要按当前文件内容生成。模型可以随 Token 一起提出有界 proposal，proposal 必须再次声明一个获授权目标、修改前摘要和完整替换内容。adapter 会先把 `PREPARED` nonce 记录刷盘，再对普通文件做受控原子替换，最后追加 `APPLIED` 记录。修改前摘要不匹配、目标不在授权集合、路径越界、符号链接、不同 nonce 重用或日志损坏都会拒绝。
...
- 独立的测试世界和判别器；
- 更严格的归因、信用分配和长期记忆机制；
- F-129/F-130 在当时版本中证明：`signed-v1` 效用方向可以安全穿过 WorldPort→Kernel→账本→Replay，但仅开放价值模式尚未让跨期套利的 horizon 8 优于 horizon 1；F-275 的当前版本复验已观察到短窗口改善，真正未解决的仍是多 seed、延迟反馈和动作链信用的长期区分；
...
- F-138 已验证独立见证路径：见证进程只接收反馈元数据和成员 nonce，不接收主 adapter 的 delta 声明；正确见证可跨 init→run→Replay 进入学习，独立签名但 delta 不一致时在第三步前拒绝且不生成模型。F-139 又验证了缺成员同样 fail-closed，但两个不同密钥对同一错误闭合份额共同签名时，v32 仍会学习错误的 `0.9/0.1` 且 Replay 一致；这确认来源独立不等于因果真实；
...
需要比较同一 WorldPort 作用域下的多次独立运行时，可以按平台路径分隔符传入多个 Lab。Windows 使用分号，Unix 使用冒号：

```powershell
yi-agent experiment counterfactual-corpus `
...
F-241 收紧跨 Run 接续时的上一 Run 校验。`startRun()` 现在逐事件消费上一 Run 的流式账本，只保留首事件、current 水位所指事件和事件计数，再完成 current 引用与状态投影校验；不再把上一 Run 的完整事件数组装入内存。运行中 current 仍返回 `BUSY`，完整事件、摘要链、状态连续性和终态规则保持原有校验。这个节点减少的是长 Run 接续的事件载荷驻留，不改变 `readRun()` 等数组兼容接口，也不消除全历史 nonce 精确校验的成本。

F-242 收紧未决外部事务的目录扫描。该恢复路径现在两遍使用异步目录迭代器；只有第一遍发现未决 terminal evidence 时才进行第二遍匹配相关 STEP，并按 `runId` 排序未决项以保持旧的首项和冲突语义。正常没有未决事务的历史不再额外保留全部 Run 名称或扫描身份集合。未决项本身和相关匹配集合仍只在异常恢复窗口存在，外部副作用仍遵守原有 nonce、对账和人工处理边界。

F-243 收紧崩溃恢复的事件载荷驻留。`recoverRun()` 现在逐行消费 `readLedgerStream()`，只留下首事件、末事件、current 水位事件、最后 STEP 状态和计数，再完成 current、end、外部 marker 与终态处理；追加 `CRASH_HALTED` 后只更新摘要，不重新构造历史数组。活动 Run 的无换行尾部先用固定大小反向扫描定位完整前缀，完整校验通过后才截断并恢复。数组版 `readLedger()` 与 `readRun()` 仍保持兼容，单个 ledger 的大小限制和精确 nonce 校验也没有被放宽。Runtime 恢复矩阵 `82/82`、应用层连续/目标/重启/多 WorldPort 回归 `29/29` 通过。
...
F-258 在同一 ESS 实验中复测中间链长。修正释放反馈的快照时点后，3 seed、h8、96 步下 pair/multi2/multi3 均值为 `13867/+1.65%`、`13962/+2.35%`、`13684/+0.31%`（baseline `13642`）。multi3 优于 pair，multi2 则高于二者；三种配置都未低于待机基线，结果不支持“中间链长最优”。12 条正式运行 Replay 均为 CONSISTENT。

F-259 使用修正后的链反馈切点重测价值投影：共享 alpha/beta/gamma seed、h8、96 步下，`distance-v2` 的 base/pair 均值为 `13527/-0.84%`、`13514/-0.94%`，`signed-v1` 为 `14015/+2.74%`、`13767/+0.92%`。链边际分别为 `-13` 和 `-248`；utility-pair 有 2/3 seed 成本下降，distance-pair 为 1/3，但两组均值都不足以证明稳定套利。12 条 CLI 运行全部 Replay CONSISTENT，四分支 E2E 确认反馈快照与当前动作前观察一致。旧 F-257～F-259 数据使用了动作后的释放快照，现已由 chain-v2 结果取代。

F-260/R15 在 `signed-v1`、h8、96 步下比较 20 组固定标签配对 seed。40 条 CLI Run 全部完成并 Replay 一致，R14 三个锚点复现。20 组初始 RNG 状态互不相同且组内配对一致；但 17 个新增标签映射为两个相邻状态段（`2049547814–2049547822` 和 `2049547844–2049547851`），不是随机抽取的样本。链组平均成本高出 `130.25` 元，3 组更低、17 组更高。名义 95% t 区间 `[5.79, 254.71]` 只描述这组固定 seed，不能当作总体区间；3 种 base 和 12 种链动作轨迹是覆盖信息，重复本身不能证明样本相关。R15 因而没有确认链信用的稳定收益或伤害。下一轮应以 CSPRNG 生成高熵 seed 标签，记录派生的实际状态并拒绝重复，再检验不同世界状态下的效果。
