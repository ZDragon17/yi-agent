...
F-259 检验价值投影与链信用的交互：共享 alpha/beta/gamma seed、h8、96 步下，默认 `distance-v2` 的 base/pair 均值为 `13527/-0.84%`、`13392/-1.83%`，累计 `signed-v1` 为 `14015/+2.74%`、`13942/+2.20%`。累计成本维度没有带来稳定套利，链边际也从 `-135` 降为 `-73`；12 条真实 CLI 运行全部 Replay CONSISTENT。新增四分支 CLI 回归 `1/1`，确认价值模式持久化、动作链结算与 Replay。这是对“增加观测维度就会产生长期智能”的负结果，不是对任意价值投影的普遍否定。
