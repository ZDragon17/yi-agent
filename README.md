...
- 已知边界（开放反证入口）：长跑中 6/7 平台在数百步后发生不可逆瓦解。步骤级账本诊断确认因果链：稳定轨道内一次价值选择意外打破周期性，上下文键失配使反事实探测与 EXPLORATORY 回合注入噪声，短暂重组失败后进入「意外→机制切换→恢复期噪声锁死」的元稳定失败；F-40 强制重验被证实为放大器而非扳机并已门控，恢复期上下文证据的再积累机制仍未解决；
