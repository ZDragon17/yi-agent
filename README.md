...
F-260/R15 将 `signed-v1`、h8、96 步的链组与 base 组扩到 20 组配对 seed。40 条 CLI Run 全部完成并 Replay 一致，R14 的三组锚点也精确复现。链组 seed 加权成本均值高出 `130.25` 元（3 组更低、17 组更高），但 20 个 seed 只有 3 种 base 动作轨迹、12 种配对轨迹，名义 95% t 区间不能作为推断。轨迹簇内效果方向不一致，因此结论仍是未确认链信用带来稳定收益；仅增加相似 seed 数量不够，下一步需扩大世界状态变化或建立真正独立的策略对照。
