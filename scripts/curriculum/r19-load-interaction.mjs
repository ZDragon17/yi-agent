...
// R19：同一 seed 的四格实验，估计负荷曲线压力与动作链的差中差交互。
// 唯一世界变量是第 48 小时起 load ×1.2；电价保持静态，ESS 参数不变。
...
import { summarizeR19FactorialRuns } from './r19-load-interaction-statistics.mjs';
import { LOAD_SCALE, LOAD_SHIFT_AT, loadForHour } from './r19-load-interaction-model.mjs';

const ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '../..');
const STEPS = 96;
const HORIZON = 8;
const SAMPLE_SIZE = 20;
const REPORT_PATH = path.join(ROOT, 'docs/figures/r19-load-interaction.json');
const STATE_SCHEMA_VERSION = 1;
const CONDITIONS = [
  { label: 'normal-base', chain: false, stress: false },
  { label: 'normal-chain', chain: true, stress: false },
  { label: 'stress-base', chain: false, stress: true },
  { label: 'stress-chain', chain: true, stress: true },
];
const SOURCE_PATHS = [
  'scripts/curriculum/r19-load-interaction-model.mjs',
  'scripts/curriculum/r19-load-interaction-statistics.mjs',
  'scripts/curriculum/r19-load-interaction.mjs',
];
...
function measureRun({ events, tokenEntries, seed, condition }) {
  ...
  for (const event of steps) {
    ...
    costYuan += Math.max(0, loadForHour(hour, condition.stress) + essPower) * staticTariffPriceAt(hour);
...
async function runCondition({ seed, condition, state, attempt }) {
  ...
  writeFileSync(adapter, JSON.stringify({
    ...
    args: [
      ...
      '--utility-mode',
      ...(condition.stress ? ['--load-shift-at', String(LOAD_SHIFT_AT), '--load-scale', String(LOAD_SCALE)] : []),
  ...
  const expectedIdentity = condition.stress ? `load-scale-${LOAD_SCALE}-at-${LOAD_SHIFT_AT}` : 'utility-v1';
...
function createCandidate(sequence, knownLabels = new Set()) {
  let seedLabel;
  do seedLabel = `r19-${randomBytes(32).toString('hex')}`; while (knownLabels.has(seedLabel));
...
function experimentDesign() {
  return {
    ...
    valueMode: 'signed-v1',
    cells: CONDITIONS.map(({ label, chain, stress }) => ({ label, chain, loadShiftAtHour: stress ? LOAD_SHIFT_AT : null, loadScale: stress ? LOAD_SCALE : 1 })),
    changedWorldCondition: `load is multiplied by ${LOAD_SCALE} from hour ${LOAD_SHIFT_AT}; tariff, ESS, and all other settings remain fixed`,
    primaryOutcome: '(stress-chain - stress-base) - (normal-chain - normal-base); positive means load stress increases chain cost relative to base',
    ...
    objectiveOracle: 'STEP action token + independently encoded stressed load + public static TOU tariff; adapter settlement totals are not used',
    replayRequired: true,
  };
}
...
function createRunState() {
  ...
  return { schemaVersion: STATE_SCHEMA_VERSION, experiment: 'R19-load-interaction', status: 'RUNNING', createdAt: new Date().toISOString(), nodeVersion: process.version, sourceFingerprint: sourceFingerprint(), artifactRoot: mkdtempSync(path.join(tmpdir(), 'r19-load-artifacts-')), design: experimentDesign(), candidates };
}
...
function validateRunState(state, expectedFingerprint) {
  ...
  if (state?.schemaVersion !== STATE_SCHEMA_VERSION || state.experiment !== 'R19-load-interaction' || state.status !== 'RUNNING' || state.nodeVersion !== process.version || state.sourceFingerprint !== expectedFingerprint || JSON.stringify(state.design) !== JSON.stringify(experimentDesign()) || !Array.isArray(state.candidates) || state.candidates.length < SAMPLE_SIZE || artifactRelativePath === '' || artifactRelativePath === '..' || artifactRelativePath.startsWith(`..${path.sep}`) || path.isAbsolute(artifactRelativePath)) {
    throw new Error('R19 run state does not match this source, Node runtime, design, or active experiment.');
  }
}
...
async function runExperiment(state, statePath) {
  ...
  console.log(`R19 state ${statePath}`);
  console.log(`R19 source fingerprint ${state.sourceFingerprint}`);
  ...
  while (acceptedRuns(state).length < SAMPLE_SIZE) {
    ...
    const usedStates = new Set(acceptedRuns(state).map((item) => item.normalBase.initialRngState));
    ...
    if (usedStates.has(candidate.factorial.normalBase.initialRngState)) {
      ...
      writeState(statePath, state);
      console.log(`seed ${candidate.sequence} rejected: repeated initial RNG state ${candidate.factorial.normalBase.initialRngState}`);
      continue;
    }
  ...
  const effect = summarizeR19FactorialRuns(factorialRuns);
  const report = {
    ...
    design: state.design,
    effect,
    worldCondition: { loadShiftAtHour: LOAD_SHIFT_AT, loadScale: LOAD_SCALE },
    runs: factorialRuns.map((run) => Object.fromEntries(Object.entries(run).map(([key, value]) => [key, ['normalBase', 'normalChain', 'stressBase', 'stressChain'].includes(key) ? publicRun(value) : value]))),
    rejectedCandidates: state.candidates.filter((candidate) => candidate.status === 'REJECTED_DUPLICATE_INITIAL_RNG_STATE').map((candidate) => ({ sequence: candidate.sequence, seedLabel: candidate.seedLabel, initialRngState: candidate.factorial.normalBase.initialRngState, rejection: candidate.rejection, treatmentOrder: candidate.treatmentOrder })),
  };
  ...
  writeState(statePath, state);
  console.log(`mean same-seed load interaction ${effect.meanInteractionDeltaYuan} yuan; decision ${effect.decision}`);
  ...
  console.log(`written docs/figures/r19-load-interaction.json; ${effect.nSeedRuns} accepted seeds, ${report.rejectedCandidates.length} repeated states rejected`);
}
...
async function main() {
  ...
  if (resumePath !== undefined) {
    ...
    statePath = requestedStatePath ?? path.join(tmpdir(), `yi-agent-r19-${randomUUID()}.json`);
