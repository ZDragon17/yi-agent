...
// R18：同一 seed 的四格实验，估计电价翻转与动作链的差中差交互。
...
import { summarizeR18FactorialRuns } from './r18-chain-tariff-interaction-statistics.mjs';
import { TARIFF_SHIFT_AT, tariffPriceAt } from './r17-chain-tariff-model.mjs';

const ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '../..');
const STEPS = 96;
const HORIZON = 8;
const SAMPLE_SIZE = 20;
const REPORT_PATH = path.join(ROOT, 'docs/figures/r18-chain-tariff-interaction.json');
const STATE_SCHEMA_VERSION = 1;
const CONDITIONS = [
  { label: 'static-base', chain: false, shift: false },
  { label: 'static-chain', chain: true, shift: false },
  { label: 'flip-base', chain: false, shift: true },
  { label: 'flip-chain', chain: true, shift: true },
];
const SOURCE_PATHS = [
  'scripts/curriculum/r17-chain-tariff-model.mjs',
  'scripts/curriculum/r18-chain-tariff-interaction-statistics.mjs',
  'scripts/curriculum/r18-chain-tariff-interaction.mjs',
];
...
function measureRun({ events, tokenEntries, seed, condition }) {
  ...
  for (const event of steps) {
    ...
    const price = condition.shift ? tariffPriceAt(hour) : staticTariffPriceAt(hour);
    costYuan += Math.max(0, loadKw(hour) + essPower) * price;
...
async function runCondition({ seed, condition, state, attempt }) {
  ...
  writeFileSync(adapter, JSON.stringify({
    ...
    args: [
      ...
      '--utility-mode',
      ...(condition.shift ? ['--regime-shift-at', String(TARIFF_SHIFT_AT)] : []),
  ...
  const expectedIdentity = condition.shift ? `tariff-flip-${TARIFF_SHIFT_AT}` : 'utility-v1';
  if (!store.manifest.worldVersion.includes(expectedIdentity)) {
    throw new Error(`world identity does not encode ${expectedIdentity} for ${seed.sequence}/${condition.label}`);
  }
...
function createCandidate(sequence, knownLabels = new Set()) {
  let seedLabel;
  do seedLabel = `r18-${randomBytes(32).toString('hex')}`; while (knownLabels.has(seedLabel));
...
function experimentDesign() {
  return {
    ...
    valueMode: 'signed-v1',
    cells: CONDITIONS.map(({ label, chain, shift }) => ({ label, chain, tariffShiftAtHour: shift ? TARIFF_SHIFT_AT : null })),
    primaryOutcome: '(flip-chain - flip-base) - (static-chain - static-base); positive means the tariff flip increases the chain cost relative to base',
    ...
    objectiveOracle: 'STEP action token + deterministic load + independently encoded tariff reversal + public TOU tariff; adapter settlement totals are not used',
    replayRequired: true,
  };
}
...
function createRunState() {
  ...
  return { schemaVersion: STATE_SCHEMA_VERSION, experiment: 'R18-chain-tariff-interaction', status: 'RUNNING', createdAt: new Date().toISOString(), nodeVersion: process.version, sourceFingerprint: sourceFingerprint(), artifactRoot: mkdtempSync(path.join(tmpdir(), 'r18-chain-tariff-artifacts-')), design: experimentDesign(), candidates };
}
...
function validateRunState(state, expectedFingerprint) {
  ...
  if (state?.schemaVersion !== STATE_SCHEMA_VERSION || state.experiment !== 'R18-chain-tariff-interaction' || state.status !== 'RUNNING' || state.nodeVersion !== process.version || state.sourceFingerprint !== expectedFingerprint || JSON.stringify(state.design) !== JSON.stringify(experimentDesign()) || !Array.isArray(state.candidates) || state.candidates.length < SAMPLE_SIZE || artifactRelativePath === '' || artifactRelativePath === '..' || artifactRelativePath.startsWith(`..${path.sep}`) || path.isAbsolute(artifactRelativePath)) {
    throw new Error('R18 run state does not match this source, Node runtime, design, or active experiment.');
  }
}
...
function acceptedRuns(state) {
  return state.candidates.filter((candidate) => candidate.status === 'ACCEPTED').map((candidate) => candidate.factorial);
}
...
function conditionResultKey(label) {
  return label.replace(/-([a-z])/gu, (_, letter) => letter.toUpperCase());
}
...
async function runExperiment(state, statePath) {
  ...
  console.log(`R18 state ${statePath}`);
  console.log(`R18 source fingerprint ${state.sourceFingerprint}`);
  ...
  while (acceptedRuns(state).length < SAMPLE_SIZE) {
    ...
    const usedStates = new Set(acceptedRuns(state).map((item) => item.staticBase.initialRngState));
    ...
    if (usedStates.has(candidate.factorial.staticBase.initialRngState)) {
      ...
      writeState(statePath, state);
      console.log(`seed ${candidate.sequence} rejected: repeated initial RNG state ${candidate.factorial.staticBase.initialRngState}`);
      continue;
    }
  ...
  const effect = summarizeR18FactorialRuns(factorialRuns);
  const report = {
    ...
    design: state.design,
    effect,
    worldCondition: { tariffShiftAtHour: TARIFF_SHIFT_AT, pricesBeforeShift: { valley: 0.35, flat: 0.7, peak: 1.2 }, pricesAfterShift: { valley: 1.2, flat: 0.7, peak: 0.35 } },
    runs: factorialRuns.map((run) => Object.fromEntries(Object.entries(run).map(([key, value]) => [key, key === 'staticBase' || key === 'staticChain' || key === 'flipBase' || key === 'flipChain' ? publicRun(value) : value]))),
    rejectedCandidates: state.candidates.filter((candidate) => candidate.status === 'REJECTED_DUPLICATE_INITIAL_RNG_STATE').map((candidate) => ({ sequence: candidate.sequence, seedLabel: candidate.seedLabel, initialRngState: candidate.factorial.staticBase.initialRngState, rejection: candidate.rejection, treatmentOrder: candidate.treatmentOrder })),
  };
  ...
  writeState(statePath, state);
  console.log(`mean same-seed tariff interaction ${effect.meanInteractionDeltaYuan} yuan; decision ${effect.decision}`);
  ...
  console.log(`written docs/figures/r18-chain-tariff-interaction.json; ${effect.nSeedRuns} accepted seeds, ${report.rejectedCandidates.length} repeated states rejected`);
}
...
async function main() {
  ...
  if (resumePath !== undefined) {
    ...
    statePath = requestedStatePath ?? path.join(tmpdir(), `yi-agent-r18-${randomUUID()}.json`);
