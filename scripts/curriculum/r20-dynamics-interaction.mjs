...
// R20：同一 seed 的四格实验，估计 ESS 充放电动力学与动作链的差中差交互。
// 唯一世界变量是电池效率 0.95 → 0.85；负荷、电价、规划范围和步数不变。
...
import { BASE_BATTERY_EFFICIENCY, BATTERY_EFFICIENCY, batteryStepAtEfficiency } from './r20-dynamics-interaction-model.mjs';
import { summarizeR20FactorialRuns } from './r20-dynamics-interaction-statistics.mjs';

const ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '../..');
const STEPS = 96;
const HORIZON = 8;
const SAMPLE_SIZE = 20;
const REPORT_PATH = path.join(ROOT, 'docs/figures/r20-dynamics-interaction.json');
const STATE_SCHEMA_VERSION = 1;
const CONDITIONS = [
  { label: 'baseline-base', chain: false, dynamic: false },
  { label: 'baseline-chain', chain: true, dynamic: false },
  { label: 'dynamic-base', chain: false, dynamic: true },
  { label: 'dynamic-chain', chain: true, dynamic: true },
];
const SOURCE_PATHS = [
  'scripts/curriculum/r20-dynamics-interaction-model.mjs',
  'scripts/curriculum/r20-dynamics-interaction-statistics.mjs',
  'scripts/curriculum/r20-dynamics-interaction.mjs',
];
...
function measureRun({ events, tokenEntries, seed, condition }) {
  ...
  for (const event of steps) {
    ...
    const efficiency = condition.dynamic ? BATTERY_EFFICIENCY : BASE_BATTERY_EFFICIENCY;
    const expectedSoc = accepted ? batteryStepAtEfficiency(soc, essPower, efficiency) : soc;
    const observedSoc = event.payload.afterState.worldState.soc;
    if (observedSoc !== expectedSoc) {
      throw new Error(`SOC oracle mismatch for ${seed.sequence}/${condition.label}: expected ${expectedSoc}, got ${observedSoc}`);
    }
    soc = observedSoc;
    // Cost is intentionally recomputed from the public action and fixed world model;
    // the ESS efficiency affects the policy trajectory through SOC, not grid power.
    const baseLoad = loadKw(hour);
    costYuan += Math.max(0, baseLoad + (capabilityId === 'ess.charge' ? 100 : capabilityId === 'ess.discharge' ? -100 : 0)) * staticTariffPriceAt(hour);
...
async function runCondition({ seed, condition, state, attempt }) {
  ...
  writeFileSync(adapter, JSON.stringify({
    ...
    args: [
      ...
      '--utility-mode',
      ...(condition.dynamic ? ['--battery-efficiency', String(BATTERY_EFFICIENCY)] : []),
  ...
  const expectedIdentity = condition.dynamic ? `battery-efficiency-${BATTERY_EFFICIENCY}` : 'utility-v1';
  ...
  return { ...measured, replay: replayEnvelope.data.verdict, worldVersion: store.manifest.worldVersion, worldImplementationDigest: store.manifest.worldImplementationDigest, artifactPath: lab, attempt };
}
...
function createCandidate(sequence, knownLabels = new Set()) {
  let seedLabel;
  do seedLabel = `r20-${randomBytes(32).toString('hex')}`; while (knownLabels.has(seedLabel));
...
function experimentDesign() {
  return {
    ...
    valueMode: 'signed-v1',
    cells: CONDITIONS.map(({ label, chain, dynamic }) => ({ label, chain, batteryEfficiency: dynamic ? BATTERY_EFFICIENCY : BASE_BATTERY_EFFICIENCY })),
    changedWorldCondition: `ESS charge/discharge efficiency changes from ${BASE_BATTERY_EFFICIENCY} to ${BATTERY_EFFICIENCY}; load, tariff, planning horizon, and all other settings remain fixed`,
    primaryOutcome: '(dynamic-chain - dynamic-base) - (baseline-chain - baseline-base); positive means dynamics stress increases chain cost relative to base',
    ...
    objectiveOracle: 'STEP action token + independently encoded SOC dynamics + fixed public load and TOU tariff; adapter settlement totals are not used',
    replayRequired: true,
  };
}
...
function createRunState() {
  ...
  return { schemaVersion: STATE_SCHEMA_VERSION, experiment: 'R20-dynamics-interaction', status: 'RUNNING', createdAt: new Date().toISOString(), nodeVersion: process.version, sourceFingerprint: sourceFingerprint(), artifactRoot: mkdtempSync(path.join(tmpdir(), 'r20-dynamics-artifacts-')), design: experimentDesign(), candidates };
}
...
function validateRunState(state, expectedFingerprint) {
  ...
  if (state?.schemaVersion !== STATE_SCHEMA_VERSION || state.experiment !== 'R20-dynamics-interaction' || state.status !== 'RUNNING' || state.nodeVersion !== process.version || state.sourceFingerprint !== expectedFingerprint || JSON.stringify(state.design) !== JSON.stringify(experimentDesign()) || !Array.isArray(state.candidates) || state.candidates.length < SAMPLE_SIZE || artifactRelativePath === '' || artifactRelativePath === '..' || artifactRelativePath.startsWith(`..${path.sep}`) || path.isAbsolute(artifactRelativePath)) throw new Error('R20 run state does not match this source, Node runtime, design, or active experiment.');
}
...
async function runExperiment(state, statePath) {
  ...
  console.log(`R20 state ${statePath}`);
  console.log(`R20 source fingerprint ${state.sourceFingerprint}`);
  while (acceptedRuns(state).length < SAMPLE_SIZE) {
    ...
    const usedStates = new Set(acceptedRuns(state).map((item) => item.baselineBase.initialRngState));
    ...
    if (usedStates.has(candidate.factorial.baselineBase.initialRngState)) {
      ...
      writeState(statePath, state);
      console.log(`seed ${candidate.sequence} rejected: repeated initial RNG state ${candidate.factorial.baselineBase.initialRngState}`);
      continue;
    }
  ...
  const effect = summarizeR20FactorialRuns(factorialRuns);
  const report = {
    ...
    design: state.design,
    effect,
    worldCondition: { baselineBatteryEfficiency: BASE_BATTERY_EFFICIENCY, dynamicBatteryEfficiency: BATTERY_EFFICIENCY },
    runs: factorialRuns.map((run) => Object.fromEntries(Object.entries(run).map(([key, value]) => [key, ['baselineBase', 'baselineChain', 'dynamicBase', 'dynamicChain'].includes(key) ? publicRun(value) : value]))),
    rejectedCandidates: state.candidates.filter((candidate) => candidate.status === 'REJECTED_DUPLICATE_INITIAL_RNG_STATE').map((candidate) => ({ sequence: candidate.sequence, seedLabel: candidate.seedLabel, initialRngState: candidate.factorial.baselineBase.initialRngState, rejection: candidate.rejection, treatmentOrder: candidate.treatmentOrder })),
  };
  ...
  writeState(statePath, state);
  console.log(`mean same-seed dynamics interaction ${effect.meanInteractionDeltaYuan} yuan; decision ${effect.decision}`);
  ...
  console.log(`written docs/figures/r20-dynamics-interaction.json; ${effect.nSeedRuns} accepted seeds, ${report.rejectedCandidates.length} repeated states rejected`);
}
...
async function main() {
  ...
  else { statePath = requestedStatePath ?? path.join(tmpdir(), `yi-agent-r20-${randomUUID()}.json`); if (existsSync(statePath)) throw new Error(`state file already exists: ${statePath}`); state = createRunState(); writeState(statePath, state); }
