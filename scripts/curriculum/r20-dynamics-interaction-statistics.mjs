...
const CONDITIONS = [
  ...
  ['baselineChain', 'baseline-chain'],
  ['dynamicBase', 'dynamic-base'],
  ['dynamicChain', 'dynamic-chain'],
];

export function summarizeR20FactorialRuns(runs) {
  validateRuns(runs);

  const baselineDeltas = runs.map((run) => run.baselineChain.costYuan - run.baselineBase.costYuan);
  const dynamicDeltas = runs.map((run) => run.dynamicChain.costYuan - run.dynamicBase.costYuan);
  const interactions = runs.map((run) =>
    (run.dynamicChain.costYuan - run.dynamicBase.costYuan) -
    (run.baselineChain.costYuan - run.baselineBase.costYuan));

  return {
    nSeedRuns: SAMPLE_SIZE,
    meanBaselineChainDeltaYuan: round2(mean(baselineDeltas)),
    medianBaselineChainDeltaYuan: round2(median(baselineDeltas)),
    meanDynamicChainDeltaYuan: round2(mean(dynamicDeltas)),
    medianDynamicChainDeltaYuan: round2(median(dynamicDeltas)),
    ...
    uniqueInitialRngStates: new Set(runs.map((run) => run.baselineBase.initialRngState)).size,
...
function validateRuns(runs) {
  if (!Array.isArray(runs) || runs.length !== SAMPLE_SIZE) {
    throw new TypeError(`R20 requires exactly ${SAMPLE_SIZE} same-seed factorial runs.`);
  }
  ...
  for (const run of runs) {
    if (run === null || typeof run !== 'object' ||
      ...
      throw new TypeError('R20 requires unique seed labels.');
    }
    ...
    for (const [key, condition] of CONDITIONS) {
      ...
      if (result === null || typeof result !== 'object' || result.condition !== condition ||
        ...
        throw new TypeError('R20 requires four condition runs with finite costs, valid initial RNG states, and consistent Replay.');
      }
      states.add(result.initialRngState);
    }
    if (states.size !== 1) throw new TypeError('R20 conditions must share the same initial RNG state.');
    const initialState = run.baselineBase.initialRngState;
    if (initialStates.has(initialState)) {
      throw new TypeError('R20 requires unique initial RNG states across seed runs.');
    }
...
function decisionFor(interval) {
  if (interval.low > 0) return 'EVIDENCE_DYNAMICS_STRESS_INCREASES_CHAIN_COST';
  if (interval.high < 0) return 'EVIDENCE_DYNAMICS_STRESS_REDUCES_CHAIN_COST';
  return 'INCONCLUSIVE_DYNAMICS_INTERACTION';
}
