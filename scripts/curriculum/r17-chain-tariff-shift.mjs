...
// R17：只改变分时电价世界，在第 48 步翻转峰谷；其余沿用 R16 的配对策略对照。
...
const REPORT_PATH = path.join(ROOT, 'docs/figures/r17-chain-tariff-shift.json');
...
const SOURCE_PATHS = [
  ...
  'scripts/curriculum/r17-chain-tariff-shift.mjs',
];
...
function measureRun({ events, tokenEntries, seed, condition }) {
  ...
  for (const event of steps) {
    ...
    // 独立按 STEP 动作、固定负荷和翻转后的公开电价重算，不采用 adapter 结算汇总。
    costYuan += Math.max(0, loadKw(hour) + essPower) * tariffPriceAt(hour);
...
async function runCondition({ seed, condition, state, attempt }) {
  ...
  writeFileSync(adapter, JSON.stringify({
    ...
    args: [
      ...
      '--utility-mode', '--regime-shift-at', String(TARIFF_SHIFT_AT),
  ...
  if (!store.manifest.worldVersion.includes(`tariff-flip-${TARIFF_SHIFT_AT}`)) {
    throw new Error(`tariff shift missing from world identity for ${seed.sequence}/${condition.label}`);
  }
...
function createCandidate(sequence, knownLabels = new Set()) {
  let seedLabel;
  do {
    seedLabel = `r17-${randomBytes(32).toString('hex')}`;
...
function experimentDesign() {
  return {
    ...
    valueMode: 'signed-v1',
    changedWorldCondition: `TOU valley/peak prices reverse at hour ${TARIFF_SHIFT_AT}; flat price, load, ESS, and all other settings remain fixed`,
...
function createRunState() {
  ...
  const artifactRoot = mkdtempSync(path.join(tmpdir(), 'r17-chain-tariff-artifacts-'));
  return {
    schemaVersion: STATE_SCHEMA_VERSION,
    experiment: 'R17-chain-tariff-shift',
    status: 'RUNNING',
...
function validateRunState(state, expectedFingerprint) {
  const artifactRelativePath = typeof state?.artifactRoot === 'string'
    ...
    : '';
  if (state?.schemaVersion !== STATE_SCHEMA_VERSION || state.experiment !== 'R17-chain-tariff-shift' ||
    ...
    throw new Error('R17 run state does not match this source, Node runtime, design, or active experiment.');
  }
}
...
async function runExperiment(state, statePath) {
  ...
  console.log(`R17 state ${statePath}`);
  console.log(`R17 source fingerprint ${state.sourceFingerprint}`);
  ...
  while (acceptedPairs(state).length < SAMPLE_SIZE) {
    ...
    for (const conditionLabel of candidate.treatmentOrder) {
      ...
      writeState(statePath, state);
      console.log(`pair ${candidate.sequence} ${conditionLabel} ${result.costYuan} yuan; state ${result.initialRngState}; Replay ${result.replay}`);
    }
    ...
    if (usedStates.has(candidate.pair.initialRngState)) {
      ...
      writeState(statePath, state);
      console.log(`pair ${candidate.sequence} rejected: repeated initial RNG state ${candidate.pair.initialRngState}`);
      continue;
    }
  ...
  const idleBaseline = Array.from({ length: STEPS }, (_, hour) => loadKw(hour) * tariffPriceAt(hour))
  ...
  const report = {
    ...
    design: state.design,
    worldCondition: {
      tariffShiftAtHour: TARIFF_SHIFT_AT,
      pricesBeforeShift: { valley: 0.35, flat: 0.7, peak: 1.2 },
      pricesAfterShift: { valley: 1.2, flat: 0.7, peak: 0.35 },
    },
  ...
  console.log(`written docs/figures/r17-chain-tariff-shift.json; ${effect.nSeedPairs} accepted pairs, ${report.rejectedCandidates.length} repeated states rejected`);
}
...
async function main() {
  ...
  if (resumePath !== undefined) {
    ...
    statePath = requestedStatePath ?? path.join(tmpdir(), `yi-agent-r17-${randomUUID()}.json`);
