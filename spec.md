...
- 监督器对齐：如果本步结算了新的反馈，前后数值观测可能包含旧动作的后验变化；变化监督器不得把该步的 `ACTION && learnable` 回执直接当作当前动作的确认进步，而应将该步记为歧义。已结算 feedback 的 nonce 仍可按 Kernel 规则独立学习。停滞后的 `EXPLORATORY` 新策略使用 `coverage-v1` 先轮换样本较少的安全候选，再比较不确定度；旧策略缺少该字段时保持 `uncertainty-v1`，不改变历史 Replay。
