...
- v29 动作链信用：单条 clean feedback 可选携带 `creditChain:{schemaVersion:1,members:[{executionNonce,share}]}`。Kernel 要求反馈 nonce 是链首锚点、成员按 pending 持久顺序排列、全部仍在 pending、份额闭合为 1 且不与同批反馈重叠；只有在没有混杂且不处于共享观测边界时，才把锚点动作前观测到反馈快照的净变化按份额分别写入链成员并输出 `ACTION_CHAIN`。缺证据或结构非法时不学习或 fail-closed；该链是 WorldPort 的结构化归因声明，不是 Kernel 自动证明的因果真值。
