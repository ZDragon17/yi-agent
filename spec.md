...
- v29 动作链信用：单条 clean feedback 可选携带 `creditChain:{schemaVersion:1,members:[{executionNonce,share}]}`。Kernel 要求反馈 nonce 是链首锚点、成员按 pending 持久顺序排列、全部仍在 pending、份额闭合为 1 且不与同批反馈重叠；只有在没有混杂且不处于共享观测边界时，才把锚点动作前观测到反馈快照的净变化按份额分别写入链成员并输出 `ACTION_CHAIN`。WorldPort 必须避免把未列入 members 的动作效果计入该快照；若当前 transition 同时执行非链成员动作，通常应使用该动作执行前的快照。Kernel 只校验声明结构，不证明快照时序或因果真值。
- v30 受控加性因果证据：`creditChain` 可改用 `basis:"counterfactual-additive-v1"` 和按 pending 顺序排列的 `{executionNonce,delta}` 成员。Kernel 只在成员 delta 向量之和与反馈 nonce 的实际净变化逐维闭合时写入成员动作模型；不闭合、交互残差、混杂、共享边界或结构非法均不学习并以 `AMBIGUOUS` 结算。该证据来自 WorldPort 的受控/反事实边界，Kernel 只能验证协议和代数闭合，不能自证干预真实性；v29 share 语义保持兼容。
- v31 描述符绑定的因果证据：`creditChain` 可改用 `basis:"counterfactual-attested-v1"`，并携带 `{schemaVersion,digest,attestation}`；WorldPort 描述符的 Ed25519 公钥必须验证覆盖完整反馈快照（nonce、stateVersion、intervalId、vector、confounderCount 和不含 attestation 的 creditChain）的签名。签名、摘要或描述符不匹配时，外部协议边界拒绝该 observation；通过来源验证后仍必须经过 v30 的逐维加性闭合，签名不证明现实干预为真。v30/v29 语义保持兼容。
- v32 独立因果见证：`creditChain` 可改用 `basis:"counterfactual-independent-v1"`，主 adapter 携带 `attestation`，配置可选独立 `witness` 子进程；宿主向 witness 只发送反馈元数据和成员 nonce，不发送主 adapter 的 delta 声明。witness 使用不同描述符公钥签名同一规范化反馈与成员 delta，宿主比较两组成员后才注入 `independentAttestation`；缺 witness、签名错误或成员不一致均在外部边界拒绝。Kernel v32 要求两份 attestation 并复用 v30 加性闭合；该机制证明来源间的一致性，不证明现实因果真值。v31/v30/v29 语义保持兼容。
- 监督器对齐：如果本步结算了新的反馈，前后数值观测可能包含旧动作的后验变化；变化监督器不得把该步的 `ACTION && learnable` 回执直接当作当前动作的确认进步，而应将该步记为歧义。已结算 feedback 的 nonce 仍可按 Kernel 规则独立学习；v29 的 `ACTION_CHAIN` 结算同样只作为旧动作链证据，不确认当前动作。停滞后的 `EXPLORATORY` 新策略使用 `coverage-v1`，在单步选择和有界规划首步都先轮换样本较少的安全候选，再比较不确定度；旧策略缺少该字段时保持 `uncertainty-v1`，不改变历史 Replay。
 - 版本边界：带 `kernelLearningVersion: 3` 的 STEP 启用有界已结算反馈收据，带 `kernelLearningVersion: 4` 的 STEP 还启用有界 pending credit 窗口，带 `kernelLearningVersion: 5` 的新 STEP 还启用有界预测信念样本，带 `kernelLearningVersion: 6` 的新 STEP 还启用 feedback 按 pending 顺序规范化，带 `kernelLearningVersion: 7` 的新 STEP 还启用共享观测边界的保守归因，带 `kernelLearningVersion: 8` 的新 STEP 还启用延迟反馈与变化监督器的证据对齐，带 `kernelLearningVersion: 9` 的新 STEP 还启用有界近期变化上下文，带 `kernelLearningVersion: 10` 的新 STEP 还为新上下文历史保存单调动作序号，使延迟 feedback 结算不改变历史发生顺序，并拒绝重复、超前或缺失动作序号；带 `kernelLearningVersion: 11` 的新 STEP 还以固定大小的顺序敏感累积状态表达超过近期窗口的已验证变化；带 `kernelLearningVersion: 12` 的新 STEP 还启用基于已验证 belief samples 的第一步分支规划；带 `kernelLearningVersion: 13` 的新 STEP 进一步要求分支改变下一步决策/预期变化才计入信息价值；带 `kernelLearningVersion: 14` 的新 STEP 再要求该预期变化落在当前 ValueSpec 的价值几何内；带 `kernelLearningVersion: 29` 的新 STEP 还启用 clean feedback 的显式动作链信用；带 `kernelLearningVersion: 30` 的新 STEP 还启用 `counterfactual-additive-v1` 加性因果证据；带 `kernelLearningVersion: 31` 的新 STEP 还启用 `counterfactual-attested-v1` 的描述符公钥验签；带 `kernelLearningVersion: 32` 的新 STEP 还要求 `counterfactual-independent-v1` 同时获得主 adapter 与独立 witness 的签名和成员一致性。v12 之前的 Replay 保持旧均值规划，v12 保留旧主动信息规则，v13 保留未投影的决策分化规则；旧版本 Replay 移除不属于其版本的字段并保持原有上下文语义；缺少标记或标记为旧版本的历史 STEP 在 Replay 时保持旧 Memory 形状、反馈到达顺序、归因、监督器和上下文语义，不凭空补写新字段。
...
- 性能：模拟世界 10000 步在当前开发机上 60 秒内完成，且每步证据完整写入；结果记录机型与磁盘环境。
