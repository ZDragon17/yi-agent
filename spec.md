...
- 版本边界：带 `kernelLearningVersion: 3` 的 STEP 启用有界已结算反馈收据，带 `kernelLearningVersion: 4` 的 STEP 还启用有界 pending credit 窗口，带 `kernelLearningVersion: 5` 的新 STEP 还启用有界预测信念样本，带 `kernelLearningVersion: 6` 的新 STEP 还启用 feedback 按 pending 顺序规范化，带 `kernelLearningVersion: 7` 的新 STEP 还启用共享观测边界的保守归因；缺少标记或标记为旧版本的历史 STEP 在 Replay 时保持旧 Memory 形状、反馈到达顺序和归因语义，不凭空补写新字段。
