param(
  ...
  [string]$RootPath = (Join-Path (Get-Location) 'repo-run')
)
...
$adapterConfig = Join-Path $root 'repo-adapter.json'
...
& powershell -NoProfile -ExecutionPolicy Bypass -File $configScript -RepoPath $RepoPath -OutputPath $adapterConfig | Out-Host
...
$init = Invoke-YiAgent @('init', '--lab', $lab, '--world', 'repo', '--seed', 'repo-seed', '--adapter', $adapterConfig, '--json')
$run = Invoke-YiAgent @('agent', 'run', '--lab', $lab, '--steps', '2', '--scenario', 'working-tree', '--adapter', $adapterConfig, '--kernel-only', '--json')
